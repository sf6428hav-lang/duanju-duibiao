...
【创新底线：禁止剧情结构同构】
Skill2的任务是基于Skill1提取的底层故事主线、核心抓点和原文信息，创造新的短篇故事。
对标原文时，只允许继承其底层母题、情绪价值、阅读吸引力等可复用的核心逻辑，不得复制原文的具体剧情骨架。
禁止出现“换背景不换结构”的仿写。
所谓剧情结构重合，不仅指事件相同，也包括：
- 相同或高度相似的事件顺序
- 相同的冲突来源
- 相同的人物关系承担冲突
- 相同的信息揭露顺序
...
- 相同的结尾反转或爽点来源
例如：
原文是“女儿死亡→发现有人偏爱外人→回忆自己多年牺牲→被要求顾全大局→彻底醒悟→放弃守护→高位反击”，
新故事不能仅将“皇陵/公主/龙脉”替换成“医院/女儿/医学资源”，然后继续按照相同顺序推进。
创新必须重新组织剧情因果。
在保留底层故事主线的前提下，必须主动改变多个结构维度，例如：
1. 核心冲突发生方式
2. 主角与关键人物的关系
3. 主角前期付出的具体形式
4. 对立方的行为动机
...
8. 高潮兑现方式
9. 结尾反转或爽点来源
新故事必须形成：“同一底层母题 + 不同人物关系 + 不同冲突因果 + 不同剧情发展 + 不同高潮兑现”。
判断标准：
不要只检查“人物、职业、背景有没有换”。
而要检查：如果把新故事的人名、背景、职业、道具重新替换成原文设定后，仍然能够将新故事的主要剧情节点一一对应到原文，那么说明创新仍然存在结构同构，必须重新设计。

【导语创新底线】
Skill1提供的“导语结构”只用于判断导语的组织形式，不得作为原文剧情模板进行仿写。

例如：
原文属于“正文衔接型”，只能说明新导语也可以采用“导语直接进入正文、正文继续承接”的方式。
原文属于“概括卡点型”，只能说明新导语也可以采用“前情压缩→最后落到关键节点→正文继续”的方式。
...
生成创新导语时，必须从已经确定的新故事本身寻找新的切入点。
导语首先服务于“新故事”，其次才考虑“导语结构对标”。
...
三个导语必须是同一创新故事的三个不同切入口，而不是原文导语的三个换皮版本。

【三个导语必须真正形成不同入口】
生成三个导语时，不允许只是改变场景、职业、地点、道具后继续讲同一段剧情。
三个版本至少应在“切入事件、信息起点、主角当前状态、读者首先知道的信息、最终引出的疑问”中的多个维度发生明显变化。
...
【痛点创新原则】
Skill1提取的痛点机制用于理解“原文为什么让读者产生这种情绪”，不是让新故事复制相同事件。
面对一个原文痛点，不要寻找“类似事件”进行替换，而要先理解其情绪机制，再结合新故事的人物关系、背景、梗和核心冲突，重新创造一种能够产生同等甚至更强情绪效果的新事件。

要求：学会制造痛点，而不是模仿痛点。
例如原文通过“重要的人受到严重伤害，却被另一个无关紧要的人抢走资源”制造价值排序错位。
新故事可以通过完全不同的事件制造“价值排序错位”，不要求继续使用：受伤、资源争夺、第三者、丈夫偏袒等表面元素。
优先寻找新的实现方式。
