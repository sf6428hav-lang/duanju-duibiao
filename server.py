...
你目前的任务是：读取 Skill 1 提取的【底层故事主线】，通过寻找【抓点】并进行脑洞扩散，生成 3 个完全不同的新故事方向。
注意：禁止“槽位填空式创新”！禁止把“剧情槽位”作为创新的主要生成方式，禁止机械化地给原故事套新梗、换名词！创新是基于原有底层情绪因果，自然生长出新的剧情可能。

【什么是抓点与梗融合】
1. 抓点：不是生硬的“槽位①②”，而是原故事中最值得继续开发的故事机制（如“长期牺牲保护大局”、“被保护的人反插一刀”、“守护者变复仇者”）。
2. 梗融合：新梗进入故事后，必须改变人物动机、人物关系、核心冲突、信息差、主角行动方式、反转或结局中至少一项。
例如加入【死人文学】，绝不仅是“主角生病了”，而是“因为生命倒计时，主角改变了原本的行动方式，不再等待，而是提前残酷布局”。

【换背景规则】
允许换背景（古言->现代豪门，现实->末世等），但换背景后必须重新设计人物动机和冲突逻辑！
绝不能只是“皇陵->公司，龙脉->风水阵，太监->秘书”的低级名词替换。必须回答“为什么这个新背景会产生一个新的故事”。

【内部推演与三道自检程序（必须在 <think> 标签内完成）】
在输出给用户前，你必须在内部完成沙盘推演，并执行三道强制自检程序。如果不过关，立刻重做！

自检程序一（剧情隔离与抓点自检）：
- 我是否找准了原故事的【核心情绪抓点】？
- 我是否摆脱了原故事的具体事件束缚，没有在做“剧情槽位填空”？

自检程序二（融合深度自检）：
- 新加入的梗是否真正改变了至少一项核心要素（动机/行动方式/关系/冲突）？
- 如果去掉这个梗，故事是不是依旧成立？（如果是，说明是无效废梗，立刻剔除！）

自检程序三（防换皮与浑水摸鱼自检，最高优先级！）：
- 检查当前生成的故事：是不是只是把原故事拆成几个槽位重新排列？
- 是不是只是单纯换了一层背景名词（总裁/皇帝），但事件发生顺序完全没变？
- 读者看完会觉得“这是从原故事长出来的一个完全不同的新故事”，还是觉得“这不就是原版换了批人吗”？
【只要发现自己在机械拼凑或名词替换，立即在内部推翻重写！我要保质保量，绝不输出为了糊弄用户的废话方案！】

【正式输出格式】
结束 <think> 思考后，请严格按以下精简格式输出 3 个方向。
【绝对禁止】：在输出中出现“剧情槽位”、“槽位①”等机械化术语内容！只输出对用户有价值的剧情方案。

### 创新方向 A：[用简短的话概括方向特点，如：现代豪门+死人文学]
- **【创新方式】**：（如：换背景+叠新梗 / 换视角 / 叠加新梗 等）
- **【使用梗】**：（明确调用了哪些 1-3 个梗）
- **【为什么适合这个底层主线】**：（说明从原主线的哪个“抓点”生长出来的，两者有何契合度）
- **【脑洞融合】**：（详细解释新梗进入后，人物动机或事件发生了什么实质性的改变）
- **【新故事主线】**：（用自然语言完整概括新的故事梗概）
- **【核心新爽点】**：（说明这个版本新的爽点体验是什么）
...
生成新故事前，必须先在内部（<think> 标签中）把原文中的具体内容全部抽象成“功能”。
