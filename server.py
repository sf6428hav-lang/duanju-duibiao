...
你将读取 Skill 1 提取的【底层故事主线】，在保留该底层因果关系（核心情绪与骨架）的前提下，引入新的创作变量，生成 3 个完全不同、更新、更有戏的新故事方向。

【创新核心原则】（必须严格遵守）
1. 绝对不要改写原故事，也不要根据原文剧情逐项换皮。
2. 绝对不要去判断“原故事缺什么”、“找修补缺陷”，更不能随机增加毫无因果的梗。
3. 你的唯一标准是：“加入这个新梗/背景后，能把原主线改造成什么更新、更有戏的新版本？”
4. 梗是变量，主线是骨架。唯一不能变的是底层故事的情绪因果；而变量必须用来改变事件的呈现方式、人物动机或反转方式。
5. 【关键因果改变原则】：梗和主线融合后，必须改变至少一个关键因果！例如不能只是“加了真假千金”，而必须明确“因为真假千金，为什么丈夫/家族会做出原本不会做的选择？”。如果加了毁容，必须是因为毁容导致了特定的行动（如退居幕后导致被低估），而不能只是单纯塞个毁容设定。
6. 【克制叠梗原则】：绝对不要认为“梗越多越好”。一次迭代默认只加 1～3 个新梗。如果只是增加了一个设定、道具或身份标签，而故事走向完全不受影响，则判定为【无效叠梗】！新增梗必须至少改变“人物动机、核心冲突、关键转折、高潮方式、结局方式”中的一项。

【允许的迭代方式】
1. 叠加新梗：如死人文学（提供生命倒计时的行动压迫感）、系统、重生等。新梗必须真正进入因果链。
2. 换背景：古言改现代、豪门改娱乐圈等。背景变化必须改变故事表达，但底层逻辑仍成立。
3. 换背景+叠新梗：重点创新方式，重新推演人物动机和事件因果。
4. 换视角：如丈夫视角、反派视角。利用信息差制造新体验，核心故事仍成立。
5. 改变反转方式：重新设计主角为什么赢、反派为什么输。
（没有哪一种是唯一正确答案，确保这 3 个方向采用不同的迭代方式组合。）

【内部思考规则】
在 <think> 标签内，构思 3 个方向。
每次构思时，必须严格自查：
- 梗的数量是否严格控制在 1-3 个？
- 新梗是否真正改变了至少一个关键因果（动机/冲突/转折/高潮/结局）？是因为新梗导致了什么原本不会发生的改变？
- 底层的核心情绪因果（如：牺牲->被背叛->反杀夺回）是否依然成立？
如果有任何无效叠梗（为了加梗而加梗），立即在 <think> 内推翻重构。

【正式输出格式】
结束 <think> 思考后，请严格按以下格式输出 3 个创新方向（方向A、B、C）：

### 创新方向 A：[用简短的一句话概括方向特点，如：现代豪门+死人文学]
- 【原底层主线】：（简述保留的核心因果）
- 【迭代方式】：（如：换背景+叠新梗）
- 【新增元素】：（明确调用了哪 1-3 个梗）
- 【融合后的逻辑】：（详细解释新元素如何切实改变了原有的因果链与人物选择！必须写出因为新梗发生了什么原本不会发生的改变）
- 【新的故事主线】：（用完整自然语言重新概括新故事梗概）
- 【核心变化】：（总结相比原故事，人物动机、冲突、转折或爽点究竟发生了什么本质改变）
...
=====================
最后，必须输出一个供用户选择的交互卡片。
【严重警告】：绝对不允许直接照抄下面代码里的“方向A名称”！你必须把你刚才生成的三个【真实创新方向的名字】填入 options 数组里！
[TEMPLATEJSON]
{
  "questions": [
    {
...
[TEMPLATEJSON]
{
  "questions": [
    {
      "question": "短篇小说章节规划已完成！请确认大纲，确认无误后可进入 Skill 4 生成正文：",
      "options": [
