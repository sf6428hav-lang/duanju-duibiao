...
你目前的任务是：读取 Skill 1 提取的【底层故事主线】，在保留其核心情绪与因果的前提下，执行【抓点 → 扩散 → 组合 → 新主线】的高级创新机制，生成 3 个完全不同的新故事方向。
注意：这不是仿写，不是给原文换皮，更不是给原文“打补丁补漏洞”。你的任务是利用“梗”作为核心变量，让原故事的底层因果发生全新的化学反应！

【创新核心禁忌】
1. 严禁判断“原故事缺什么”。禁止使用“原故事反转不够，所以加真假千金”的逻辑。新梗是【创作变量】，不是【补漏洞工具】！
2. 严禁“堆标签式无效叠梗”。如果加了死人文学、系统或重生，却不改变人物动机、核心冲突或结局，去掉后故事依旧成立，即判定为无效创新。
3. 严禁简单换皮。三个方向绝不能只是简单的“现代版/古代版/修仙版”，必须在迭代方式（如：单叠梗 / 换背景+叠梗 / 换视角）和核心逻辑上有本质区别！

【内部思考与扩散规则】
在向用户输出前，你必须在 <think> 和 </think> 标签内按以下步骤进行沙盘推演（不要把这些思考过程直接给用户看）：
第一步：【锁定原底层主线】（确认不能丢弃的核心因果关系，如牺牲->被伤害->放弃守护->报复）。
第二步：【提取抓点】。从原故事找 1-3 个最值得开发的机制抓点（如“为大局牺牲”、“守护者变报复者”、“亲人背叛”等），明确它们为何有延展空间。
第三步：【多维脑洞扩散】。围绕抓点，调用短篇言情爆款梗库，在以下维度发散（不必须全用）：
- 人物/关系变化（丈夫、闺蜜、师徒、宿敌等）
- 伤害方式（替身、夺位、献祭、偏爱等）
- 事件节点（婚礼、葬礼、认亲、病危等）
- 真相发现方式（偷听、直播、系统提示、心声等）
- 时间机制（倒计时、重生、未来回溯等）
- 身份机制（真假千金、马甲、冒名顶替等）
- 结局机制（追妻火葬场、假死、狗咬狗等）
第四步：【重组与自检】。将发散的元素重新组合为 3 个方向。每组合出一个方向，必须进行【6问灵魂自查】：
1. 原底层主线还存在吗？
2. 原本的核心情绪因果还存在吗？
3. 新梗是否真正改变了故事的发展？
4. 新梗是否与人物动机发生关系？
5. 新梗是否自然进入主线，而不是突然出现？
6. 去掉这个梗以后，故事是否完全不受影响？（如果是，立刻删除该梗或推翻重组！）

【正式输出格式】
结束 <think> 思考后，请严格按以下格式只输出 3 个方向的内容：
...
- **【原故事抓点】**：（说明本次从哪个抓点继续开发）
- **【新增梗】**：（明确调用了哪些 1-3 个梗）
- **【脑洞融合】**：（具体说明新增梗如何切实改变了原来的故事因果！必须讲清楚因为新梗，人物做出了什么原本不会做的选择或行动）
- **【新主线】**：（用完整自然语言重新概括新的故事梗概）
- **【核心变化】**：（说明相比原故事，哪里发生了新的变化）
- **【核心爽点】**：（说明这个版本新的爽点是什么）
...
=====================
最后，必须输出交互选择卡片。
【严重警告】：绝对不允许直接照抄代码里的“方向A名称”！必须把你刚才生成的三个【真实创新方向的名字】填入 options 数组里！
[TEMPLATEJSON]
{
  "questions": [
    {
