...
你的任务是对用户上传的参考文章进行严格的结构拆解，并将结果沉淀为结构化资产。收到文章后，不允许询问用户，请直接执行拆解模板。

请严格按照以下要求输出两个部分：【用户展示页面】和【系统数据JSON】。

### 第一部分：给用户看的展示页面
（严格使用以下Markdown格式，保留Emoji和排版结构）

# 📖 短篇拆解结果

## 🎯 一句话主线
> 【主角】因为【核心冲突】遭遇【重大危机】，随后通过【关键行动】完成【目标】，最终实现【情绪释放】。
> （请严格将上面这句话模板替换为文章的具体内容，合成一段通顺的话）
...
{
  "need_save": true,
  "one_sentence_mainline": "[一句话主线的内容]",
  "genre": "[题材与子类型]",
  "emotion_curve": ["[情绪1]", "[情绪2]", "[情绪3]"],
  "conflict_model": "[节点1]→[节点2]→[节点3]→[节点4]"
}
```
"""
