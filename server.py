...
你将读取 Skill 1 提取的【底层故事主线】，在保留该底层因果关系的前提下，引入新的创作变量，生成 3 个完全不同、更新、更有戏的新故事方向。

【创新核心原则】
1. 绝对不要改写原故事，也不要根据原文剧情逐项换皮。
2. 绝对不要去判断“原故事缺什么”、“找修补缺陷”，更不能随机增加毫无因果的梗。
3. 你的唯一标准是：“加入这个新梗/背景后，能把原主线改造成什么更新、更有戏的新版本？”
4. 梗是变量，主线是骨架；变量可以改变事件的呈现方式、人物动机、反转方式，但绝对不能拆掉原底层情绪因果链！

【允许的迭代方式】
1. 叠加新梗：如死人文学（提供生命倒计时的行动压迫感，而不是单纯生病）、真假千金、重生等。新梗必须真正进入因果链，改变动机或体验。
2. 换背景：古言改现代、豪门改娱乐圈等。背景变化必须改变故事表达（不仅是把皇陵换成公司），但底层故事仍成立。
3. 换背景+叠新梗：重点创新方式，重新推演人物动机和事件因果。
4. 换视角：如丈夫视角、反派视角。利用信息差制造新体验，核心故事仍成立。
5. 改变反转方式：重新设计主角为什么赢、反派为什么输。

【内部思考规则】
在 <think> 标签内，构思 3 个方向。每次至少生成 3 个方向，但方向必须真正不同（例如：A是叠死人文学改变时间压力；B是换豪门+真假千金改变利益结构；C是换反派视角）。不允许只是换几个名词的假创新！

【正式输出格式】
结束 <think> 思考后，请严格输出以下 3 个创新方向（方向A、B、C）：

### 创新方向 A：[用简短的一句话概括方向特点，如：现代豪门+死人文学]
- 【原底层主线】：（简述保留的核心因果）
- 【迭代方式】：（如：换背景+叠新梗）
- 【新增元素】：（明确调用了哪些设定/梗）
- 【融合后的逻辑】：（解释新元素如何进入并改变原有的因果链与人物动机）
- 【新的故事主线】：（用完整自然语言重新概括新故事梗概）
- 【核心变化】：（告诉用户相比原故事，人物关系、冲突、爽点或反转发生了什么本质改变）
