    ...
    function renderPendingImages() {
      ...
      pendingImages.forEach(function(img, idx) {
        html += '<div style="position:relative; display:inline-block;">' +
                  '<img src="' + img + '" style="height:60px; border-radius:6px; border:1px solid var(--border);">' +
                  '<button onclick="pendingImages.splice(' + idx + ', 1); renderPendingImages();" style="position:absolute; top:-5px; right:-5px; background:red; color:white; border:none; border-radius:50%; width:18px; height:18px; font-size:10px; cursor:pointer; line-height:16px; padding:0;">✕</button>' +
                '</div>';
      });
      area.innerHTML = html;
    }
    ...
    // ====== 模式切换 ======
      ...
      function setMode(mode) {
        if (mode === '通用') {
            ...
            S.mode = mode;
            if (mode === '短剧对标') S.benchStep = 1;
        }
        sv(); rf();
      }

      function updateToolbarUI() {
        var currentMode = S.mode || '通用';
        ...
        if (currentMode === '通用') {
            ...
            if (selText) selText.innerText = '未选择能力';
        } else if (currentMode === '剧本创作') {
            if (elCreate) elCreate.classList.add('active');
            if (selText) selText.innerText = '✨ 剧本创作';
...
E('editorFileName').innerText = '加载中...';
          ...
          if (sel) {
            ...
            try {
              ...
              if (d2.status === 'ok' && d2.messages) {
                S.cid = cidToFetch;
                var targetC = CC();
                if (d2.messages.length > 0 || !targetC.msgs || targetC.msgs.length === 0) {
    ...
    if (resizerRight && editorPane) {
        ...
        document.addEventListener('mousemove', function(e) {
          ...
          let newWidth = e.clientX - editorPane.getBoundingClientRect().left;
